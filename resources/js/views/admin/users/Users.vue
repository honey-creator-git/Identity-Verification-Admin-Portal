...
export default {
  ...
  methods: {
    ...
    async getUsers({ page, filter, sort }) {
      try {
        ...
        if (filter) {
          ...
          console.log("Return Data 2 => ", return_data);
          return_data = exist.removeDuplicates(return_data);
        } else {
          return_data = pagination_data;
        }
    ...
    setCh() {
      ...
      this.companies = [{ value: "", text: "该地区没有公司" }];
    ...
    async getCompaniesWithRegionName(region_name) {
      ...
      if (response.data) {
        let splitedCompanies = response.data.split(" ");
        let companies = [];
        splitedCompanies.forEach((item, index) => {
          if (item != "" && item != "/")
          {
